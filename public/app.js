...
const state = { user:null, projects:[], currentProject:null, statuses:[], dashboardView:'cards', dashboardStatus:'all', projectList:'ongoing' };
...
function renderLogin(){
  ...
  document.querySelector('#loginForm').onsubmit = async (e)=>{
    ...
    try{ const r=await api('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(Object.fromEntries(fd))}); state.user=r.user; state.projectList='ongoing'; state.dashboardStatus='all'; location.hash=''; await renderRoute(); }
...
function designDetailHtml(d){
  ...
  return `${files.length?`<div class="design-images"><img class="main-image js-main-image" src="/api/files/${files[0].id}" alt="${escAttr(d.title)}"><div class="side-thumbs">${files.map(f=>`<img src="/api/files/${f.id}" data-thumb-src="/api/files/${f.id}" alt="${escAttr(f.filename)}">`).join('')}</div></div>`:''}
    <div class="detail-grid"><div><div class="eyebrow">Proposal Details</div>${d.description?`<div class="copy" style="margin:8px 0 15px">${esc(d.description)}</div>`:''}
      ...
      ${diamonds.length?`<table class="diamond-table"><thead><tr><th>Shape</th><th>Weight</th><th>#</th><th>Color / Clarity</th><th>Origin</th><th>Provided By</th><th>Measurements</th></tr></thead><tbody>${diamonds.map(x=>`<tr><td>${esc(x.shape||'—')}</td><td>${x.weight_ct==null?'—':`${num(x.weight_ct,3)} ct${x.weight_mode==='each'?' ea.':''}`}</td><td>${x.stone_count||1}</td><td>${esc(x.color_clarity||'—')}</td><td>${esc(x.diamond_origin||'—')}</td><td>${esc(x.provided_by||'—')}</td><td>${esc(x.measurements||'—')}</td></tr>`).join('')}</tbody></table>`:'<div class="muted">No diamond lines entered.</div>'}
...
function openEditProjectModal(p){
  const refs=state.currentProject?.reference_files||[];
...
function openProposalModal({projectId,design=null}){
  const editing=!!design, d=design||{}, existingDesigns=(state.currentProject?.designs||[]).filter(x=>x.id&&x.id!==d.id), designFiles=d.files||[];
  openModal(`<div class="modal-head"><h2>${editing?'Edit':'Add'} Design Proposal</h2><button class="btn btn-ghost btn-small" data-close>Close</button></div><form id="designForm" class="form-grid">
    ...
    <div class="field span-2"><label>${editing?'Proposal Images':'Design Images'}</label>${editing&&designFiles.length?`<div class="reference-editor">${designFiles.map(f=>`<label class="reference-edit-item"><img src="/api/files/${f.id}" alt="${escAttr(f.filename||'Proposal image')}"><span><input type="checkbox" name="remove_design_image_ids" value="${escAttr(f.id)}"> Delete image</span></label>`).join('')}</div>`:''}<input type="file" name="design_images" multiple ${editing?'':'required'} accept="image/*,.png,.PNG,.jpg,.JPG,.jpeg,.JPEG,.webp,.WEBP"><div class="helper">${editing?'Select existing images to delete, and choose any new images to add. Changes are applied when you save.':'The first image becomes the card thumbnail.'}</div></div>
  ...
  document.querySelector('#designForm').onsubmit=async e=>{e.preventDefault();const form=e.currentTarget;const diamonds=[...rows.querySelectorAll('.diamond-row')].map(row=>Object.fromEntries(new FormData(row))).map(x=>({...x,stone_count:Number(x.stone_count||1),weight_ct:x.weight_ct===''?null:Number(x.weight_ct)}));const findings=[...findingRows.querySelectorAll('.finding-row')].map(row=>Object.fromEntries(new FormData(row)));form.querySelector('#diamondsJson').value=JSON.stringify(diamonds);form.querySelector('#findingsJson').value=JSON.stringify(findings);const formData=new FormData(form);formData.set('remove_design_image_ids',JSON.stringify(formData.getAll('remove_design_image_ids')));const btn=form.querySelector('[type=submit]');btn.disabled=true;try{await api(editing?`/api/designs/${d.id}`:`/api/projects/${projectId}/designs`,{method:editing?'PATCH':'POST',body:formData});closeModal();toast(editing?'Proposal updated':'Proposal added');renderProject(projectId)}catch(err){toast(err.message);btn.disabled=false}};
}
...
function addDiamondRow(parent,values={}){
  const row=document.createElement('form');row.className='diamond-row';row.innerHTML=`
    ...
    <div class="field diamond-provider"><label>Provided By</label><div class="provider-options"><label><input type="radio" name="provided_by" value="Customer Provided" ${values.provided_by==='Customer Provided'?'checked':''}> Customer Provided</label><label><input type="radio" name="provided_by" value="Shivani Provided" ${values.provided_by==='Shivani Provided'?'checked':''}> Shivani Provided</label></div></div>
